...
class Human(object):
    ...
    def run(self, city):
        ...
        while True:
            ...
            #set hours spent exercising and shopping per week as 0 at the beginning of the week
            if self.env.day_of_week()==0:
                self.count_exercise=0
                self.count_shop=0
            ...
            elif hour in self.shopping_hours and self.env.day_of_week() in self.shopping_days and self.count_shop<=self.max_shop_per_week:
                self.count_shop+=1
                yield self.env.process(self.excursion(city, "shopping"))

            elif hour in self.exercise_hours and self.env.day_of_week() in self.exercise_days and self.count_exercise<=self.max_exercise_per_week:
